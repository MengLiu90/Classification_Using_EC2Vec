...
for train_index, test_index in skf.split(df, df['label']):
    ...
    rf_classifier.fit(train_set.drop(columns=['product', 'EC', 'label']), train_set['label'])

    joblib.dump(rf_classifier, f'Trained_model/product_ec_classifier_trained_model_fold_{fold_cnt+1}.pkl')

    # # load the trained model
    # rf_classifier = joblib.load(f'Trained_model/product_ec_classifier_trained_model_fold_{fold_cnt}.pkl')

    # Evaluate the Model
    X_test = test_set.drop(columns=['product', 'EC', 'label'])
